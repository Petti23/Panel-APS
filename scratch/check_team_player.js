...
async function checkTeamPlayer() {
    try {
        const { data, error } = await supabase.from('team_player').select('*').limit(1)
        if (error) {
            console.log('team_player table error:', error.message)
        } else {
            console.log('team_player columns:', data.length > 0 ? Object.keys(data[0]) : 'exists but empty')
        }
    } catch (e) {
